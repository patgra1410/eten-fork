		...
		for (const update of changesJson.Changes) {
			...
			if (update.Resource.Type === 'SchoolNotices') {
				...
				if (noticeJson.Code === 'NoticeNotFound') {
					...
					if (noticeJson.SchoolNotice.Subject.toLowerCase().search('zmiany w planie') != -1) {
					...
					if (noticeJson.SchoolNotice.Subject.toLowerCase().search('zmiany w planie') != -1 && update.Type == 'Add')
						bets.check(client)
