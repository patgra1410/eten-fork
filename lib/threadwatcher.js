...
const newPostEvent = new EventEmitter()
    ...
    for (const post of threadResultJson.posts) {
      if (!knownPosts.has(post.no)) {
        if ('tim' in post) {
          ...
          setTimeout(function () { newPostEvent.emit('newPost', board, threadID, post.no, post.com, `https://i.4cdn.org/${board}/${post.tim}${post.ext}`) }, postTimeout)
          postTimeout += desiredPostDelay
