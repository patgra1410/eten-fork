			...
			if (update.Resource.Type === 'SchoolNotices') {
				...
				if (noticeJson.Code === 'NoticeNotFound') {
					...
					if (update.Type == 'Add')
						...
						title = 'Zmienione'
					let text = (
						`**__:loudspeaker: ${title} Ogłoszenie w Librusie__**
                        **__${noticeJson.SchoolNotice.Subject}__**
                        ${noticeJson.SchoolNotice.Content}`.replace(/  +/g, '')
					)
					let textWithAllClasses = text
					const textWithoutRoles = text

					if (czyZmianyWPlanie(noticeJson.SchoolNotice.Subject)) {
					...
					for (const info of settings.notices.where) {
						...
						if (info.guild == '930512190220435516') {
							for (const split of Util.splitMessage(textWithAllClasses))
								await channel.send(split)
							continue
						}

						for (const split of Util.splitMessage((info.roles ? text : textWithoutRoles)))
							await channel.send(split)
					}

					if (czyZmianyWPlanie(noticeJson.SchoolNotice.Subject) && update.Type == 'Add') {
						bets.addTime(new Date())
						bets.check(client)
					}
